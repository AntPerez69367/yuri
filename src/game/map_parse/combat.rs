...
use crate::game::pc::{
    ...
    FLAG_MAGIC, SFLAG_FULLSTATS, SFLAG_HPMP,
};
...
use super::packet::{
    clif_send, encrypt, wfifob, wfifohead, wfifoheader, wfifol, wfifop, wfifoset, wfifow, AREA,
    SAMEAREA, SELF,
};
...
use crate::game::map_parse::chat::{clif_playsound_entity, clif_sendminitext, clif_sendmsg};
    ...
    if sd.critchance > 0 {
        ...
        if sd.player.inventory.equip[EQ_WEAP as usize].id > 0 {
            unsafe {
                clif_playsound_entity(
                    ...
                    src.y,
                    BL_PC as u8,
                    item_db::search(sd.player.inventory.equip[EQ_WEAP as usize].id).sound_hit
                        as i32,
                );
            }
        }
        ...
        for x in 0..14usize {
            if sd.player.inventory.equip[x].id > 0 {
                sl_doscript_2(
                    carray_to_str(&item_db::search(sd.player.inventory.equip[x].id).yname),
                    Some("on_hit"),
                    sd.id,
                    src.id,
                );
        ...
        for x in 0..MAX_SPELLS {
            if sd.player.spells.skills[x] > 0 {
                sl_doscript_2(
                    carray_to_str(&magic_db::search(sd.player.spells.skills[x] as i32).yname),
                    Some("passive_on_hit"),
                    sd.id,
                    src.id,
                );
        ...
        for x in 0..MAX_MAGIC_TIMERS {
                ...
                && sd.player.spells.dura_aether[x].duration > 0
            {
                ...
                if caster_id > 0 && crate::game::map_server::map_id2sd_pc(caster_id).is_some() {
                    dispatch(
                        spell_root,
                        Some("on_hit_while_cast"),
                        &[sd.id, src.id, caster_id],
                    );
                } else {
        ...
        if let Some(src_arc) = map_id2sd_arc(src.id) {
            unsafe {
                clif_sendstatus(&src_arc, SFLAG_HPMP);
            }
        }
    }
...
///
pub fn clif_send_pc_healthscript(sd: &mut MapSessionData, damage: i32, critical: i32) -> i32 {
    ...
    let bl_id = if crate::game::map_server::entity_position(attacker_id).is_some() {
        attacker_id
    } else {
        0
    };
    ...
    if sd.player.combat.hp != 0 && damage > 0 {
        ...
        for x in 0..MAX_MAGIC_TIMERS {
                ...
                && sd.player.spells.dura_aether[x].duration > 0
            {
                sl_doscript_2(
                    carray_to_str(
                        &magic_db::search(sd.player.spells.dura_aether[x].id as i32).yname,
                    ),
                    Some("on_takedamage_while_cast"),
                    sd.id,
                    bl_id,
                );
            }
    ...
    if sd.group_count > 0 {
        if let Some(sd_arc) = map_id2sd_arc(sd.id) {
            unsafe {
                clif_grouphealth_update(&sd_arc);
            }
        }
    }
...
pub fn clif_send_mobbars_inner(mob: &MobSpawnData, sd: &MapSessionData) -> i32 {
    if mob.current_vita == 0 && mob.maxvita == 0 {
        return 1;
    }
...
pub fn clif_send_mob_health_sub_inner(
    ...
    unsafe {
        ...
        if !viewer_in_group && sd.id != sd_viewer.id {
            return 0;
        }
...
pub fn clif_send_mob_health(mob: &mut MobSpawnData, damage: i32, critical: i32) -> i32 {
    ...
    if subtype == 0 {
        ...
        sl_doscript_2(
            carray_to_str(unsafe { &(*data).yname }),
            Some("on_attacked"),
            mob.id,
            attacker_id,
        );
...
pub async fn clif_send_mob_healthscript(mob: &mut MobSpawnData, damage: i32, critical: i32) -> i32 {
    ...
    if currentvita > 0 && damage > 0 {
        for x in 0..MAX_MAGIC_TIMERS {
            ...
            if p.id > 0 && p.duration > 0 {
                sl_doscript_2(
                    carray_to_str(&magic_db::search(p.id as i32).yname),
                    Some("on_takedamage_while_cast"),
                    mob.id,
                    bl_id,
                );
            ...
            if let Some(grid) = block_grid::get_grid(mob.m as usize) {
                ...
                for id in ids {
                    if let Some(pc_arc) = crate::game::map_server::map_id2sd_pc(id) {
                        ...
                        clif_send_mob_health_sub_inner(
                            &mut pc, &mut *sd, mob, critical, pct_int, damage,
                        );
                    }
                }
            }
    ...
    if mob.current_vita == 0 {
        ...
        for x in 0..MAX_MAGIC_TIMERS {
            if mob.da[x].id > 0 && mob.da[x].duration > 0 {
                sl_doscript_2(
                    carray_to_str(&magic_db::search(mob.da[x].id as i32).yname),
                    Some("before_death_while_cast"),
                    mob.id,
                    bl_id,
                );
            }
    ...
    if mob.current_vita == 0 {
        unsafe {
            mob_flushmagic(mob as *mut MobSpawnData);
        }
                ...
                if tmob.is_null() {
                    ...
                    for x in 0..MAX_MAGIC_TIMERS {
                        if sd_ref.player.spells.dura_aether[x].id > 0
                            && sd_ref.player.spells.dura_aether[x].duration > 0
                        {
                            sl_doscript_2(
                                carray_to_str(
                                    &magic_db::search(
                                        sd_ref.player.spells.dura_aether[x].id as i32,
                                    )
                                    .yname,
                                ),
                                Some("on_kill_while_cast"),
                    ...
                    for x in 0..MAX_SPELLS {
                        if sd_ref.player.spells.skills[x] > 0 {
                            sl_doscript_2(
                                carray_to_str(
                                    &magic_db::search(sd_ref.player.spells.skills[x] as i32).yname,
                                ),
                                Some("passive_on_kill"),
                                sd_bl_id,
                                mob.id,
                            );
                        }
...
pub fn clif_parsemagic(sd: &mut MapSessionData) -> i32 {
    ...
    if i > 0 {
        ...
        if let Some(sd_arc) = map_id2sd_arc(sd.id) {
            unsafe {
                clif_sendminitext(&sd_arc, msg.as_ptr() as *const i8);
            }
        }
        return 0;
    }
    ...
    if sd.silence > 0 && spell.mute as i32 <= sd.silence {
        ...
        if let Some(sd_arc) = map_id2sd_arc(sd.id) {
            unsafe {
                clif_sendminitext(&sd_arc, c"You have been silenced.".as_ptr());
            }
        }
        return 0;
    }
    ...
    if sd.target != 0 {
        ...
        let Some((tpos, tbl_type)) = crate::game::map_server::entity_position(target_id) else {
            return 0;
        };
        ...
        if tbl_type == BL_PC {
            ...
            if !tsd2.is_null()
                && unsafe { (*tsd2).optFlags } & crate::game::pc::OPT_FLAG_STEALTH != 0
            {
                return 0;
            }
        }
        ...
        if crate::game::util::check_proximity(one, two, 21) {
            ...
            if spell.canfail as i32 == 1 {
                ...
                if cast_test < fail_chance {
                    if let Some(sd_arc) = map_id2sd_arc(sd.id) {
                        unsafe {
                            clif_sendminitext(&sd_arc, c"The magic has been deflected.".as_ptr());
                        }
                    }
                    return 0;
                }
...
pub fn clif_sendaction_pc(sd: &mut MapSessionData, action_type: i32, time: i32, sound: i32) -> i32 {
    ...
    tracing::debug!(
        "[attack] clif_sendaction: id={} action={} time={} sound={} m={} x={} y={}",
        sd.id,
        action_type,
        time,
        sound,
...
pub fn clif_sendmob_action(mob: &mut MobSpawnData, action_type: i32, time: i32, sound: i32) -> i32 {
    ...
    if sound > 0 {
        unsafe {
            clif_playsound_entity(mob.id, mob.m, mob.x, mob.y, BL_MOB as u8, sound);
        }
    }
...
pub fn clif_sendanimation_xy_inner(
    sd: &MapSessionData,
    anim: i32,
    times: i32,
    x: i32,
...
pub fn clif_sendanimation_inner(
    fd: SessionId,
    setting_flags: u32,
    anim: i32,
    target_id: u32,
...
pub fn clif_parseattack(sd: &mut MapSessionData) -> i32 {
    ...
    if sd.paralyzed != 0 || sd.sleep != 1.0f32 {
        tracing::warn!(
            "[attack] clif_parseattack BLOCKED: paralyzed={} sleep={}",
            sd.paralyzed,
            sd.sleep
        );
        return 0;
    ...
    if sd.player.combat.state == 1 || sd.player.combat.state == 3 {
        tracing::warn!(
            "[attack] clif_parseattack BLOCKED: state={}",
            sd.player.combat.state
        );
        return 0;
    }
    tracing::debug!(
        "[attack] clif_parseattack PASS: id={} atkspd={} state={}",
        sd.id,
        attackspeed,
        sd.player.combat.state
    );
    ...
    for x in 0..MAX_MAGIC_TIMERS {
        if sd.player.spells.dura_aether[x].id > 0 && sd.player.spells.dura_aether[x].duration > 0 {
            sl_doscript_simple(
                carray_to_str(&magic_db::search(sd.player.spells.dura_aether[x].id as i32).yname),
                Some("on_swing_while_cast"),
                sd.id,
            );
        }
    }
...
pub fn clif_deductdura(sd: &mut MapSessionData, equip: i32, val: i32) -> i32 {
    ...
    if sd.player.inventory.equip[equip_idx].id == 0 {
        return 0;
    }

    let m = sd.m as usize;
    if unsafe { (*raw_map_ptr().add(m)).pvp } != 0 {
        return 0;
    }
...
pub fn clif_deductweapon(sd: &mut MapSessionData, hit: i32) -> i32 {
    if unsafe { pc_isequip(sd as *mut MapSessionData, EQ_WEAP) } != 0 && rnd(100) > 50 {
...
pub fn clif_checkdura(sd: &mut MapSessionData, equip: i32) -> i32 {
    ...
    if percentage <= 0.5 && sd.player.inventory.equip[equip_idx].repair == 0 {
        unsafe {
            format_dura_msg(&mut msg_buf, item.name.as_ptr() as *mut i8, "50");
        }
    ...
    if percentage <= 0.25 && sd.player.inventory.equip[equip_idx].repair == 1 {
        unsafe {
            format_dura_msg(&mut msg_buf, item.name.as_ptr() as *mut i8, "25");
        }
        if let Some(ref arc) = sd_arc {
            unsafe {
                clif_sendmsg(arc, 5, msg_buf.as_ptr());
    ...
    if percentage <= 0.1 && sd.player.inventory.equip[equip_idx].repair == 2 {
        unsafe {
            format_dura_msg(&mut msg_buf, item.name.as_ptr() as *mut i8, "10");
        }
        if let Some(ref arc) = sd_arc {
            unsafe {
                clif_sendmsg(arc, 5, msg_buf.as_ptr());
    ...
    if percentage <= 0.05 && sd.player.inventory.equip[equip_idx].repair == 3 {
        unsafe {
            format_dura_msg(&mut msg_buf, item.name.as_ptr() as *mut i8, "5");
        }
        if let Some(ref arc) = sd_arc {
            unsafe {
                clif_sendmsg(arc, 5, msg_buf.as_ptr());
    ...
    if percentage <= 0.01 && sd.player.inventory.equip[equip_idx].repair == 4 {
        unsafe {
            format_dura_msg(&mut msg_buf, item.name.as_ptr() as *mut i8, "1");
        }
        if let Some(ref arc) = sd_arc {
            unsafe {
                clif_sendmsg(arc, 5, msg_buf.as_ptr());
    ...
    if broken {
        if item.protected != 0 || sd.player.inventory.equip[equip_idx].protected >= 1 {
            sd.player.inventory.equip[equip_idx].protected = sd.player.inventory.equip[equip_idx]
                .protected
                .saturating_sub(1);
            sd.player.inventory.equip[equip_idx].dura = item.dura;
            unsafe {
                format_restore_msg(&mut msg_buf, item.name.as_ptr() as *mut i8);
            }
            if let Some(ref arc) = sd_arc {
                unsafe {
        ...
        unsafe {
            format_destroy_msg(&mut msg_buf, item.name.as_ptr() as *mut i8);
        }
...
pub fn clif_deductduraequip(sd: &mut MapSessionData) -> i32 {
    ...
    for equip in 0..14usize {
        if sd.player.inventory.equip[equip].id == 0 {
            continue;
        }
        ...
        if broken {
            if item.protected != 0 || sd.player.inventory.equip[equip].protected >= 1 {
                sd.player.inventory.equip[equip].protected =
                    sd.player.inventory.equip[equip].protected.saturating_sub(1);
