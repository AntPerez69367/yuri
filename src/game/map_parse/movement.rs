...
use crate::game::pc::{
    MapSessionData, BL_MOB, BL_NPC, BL_PC, FLAG_FASTMOVE, FLAG_HELM, FLAG_NECKLACE,
    OPT_FLAG_GHOSTS, OPT_FLAG_STEALTH,
};
use crate::game::player::prelude::*;
use crate::session::session_exists;

use super::packet::{
    encrypt, rfifob, rfifol, rfifow, wfifob, wfifohead, wfifoheader, wfifol, wfifop, wfifoset,
    wfifow, AREA, AREA_WOS,
};
...
use crate::common::constants::entity::player::{
    ...
    EQ_NECKLACE, EQ_SHIELD, EQ_WEAP, PC_DIE_I8 as PC_DIE, PC_DISGUISE_I8 as PC_DISGUISE,
    PC_INVIS_I8 as PC_INVIS, PC_MOUNTED_I8 as PC_MOUNTED,
};
...
use crate::database::magic_db::{self};
...
use crate::game::map_parse::groups::{clif_canmove_sub_inner, clif_isingroup, clif_leavegroup};
use crate::game::map_parse::player_state::{
    clif_getchararea, clif_sendmapinfo, clif_sendstatus, clif_sendxy, clif_sendxychange,
    clif_sendxynoclick,
};
use crate::game::map_parse::visual::{
    announce_to_nearby, clif_charlook, clif_cmoblook, clif_cnpclook,
...
use crate::game::map_server::{
    map_id2mob_ref, map_id2npc_ref, map_id2sd_pc, map_readglobalreg, object_flags,
};
use crate::game::mob::{MobSpawnData, MOB_DEAD};
use crate::game::npc::NpcData;
use crate::game::pc::{pc_isequip, pc_runfloor_sub, pc_warp, FLAG_GROUP};
...
unsafe fn read_tile(m: i32, x: i32, y: i32) -> u16 {
    let Some(md) = map_data(m as usize) else {
        return 0;
    };
    if md.tile.is_null() {
        return 0;
...
fn nex_crcc(buf: &[u16]) -> u16 {
    ...
    while i + 2 < buf.len() {
        ...
        crc = ((temp << 8) ^ CRC_TABLE[((crc & 0xFF) ^ (temp >> 8)) as usize]) ^ buf[i + 2];
        i += 3;
    }
    crc
}
...
pub unsafe fn clif_charspecific(sender: i32, id: i32) -> i32 {
    ...
    let Some(sd_arc) = crate::game::map_server::map_id2sd_pc(sender as u32) else {
        return 0;
    };
    let sd: *const MapSessionData = &*sd_arc.read() as *const MapSessionData;
    let Some(src_arc) = crate::game::map_server::map_id2sd_pc(id as u32) else {
        return 0;
    };
    ...
    if shield_id == 0 {
        ...
        let (slook, scolor) =
            if (&(*sd).player.inventory.equip)[EQ_SHIELD as usize].custom_look != 0 {
                (
    ...
    let helm_item = if helm_id != 0 {
        Some(item_db::search(helm_id as u32))
    } else {
        None
    };
    let helm_look = helm_item.as_ref().map_or(-1, |i| i.look);
    if helm_id == 0 || ((*sd).player.appearance.setting_flags & FLAG_HELM) == 0 || helm_look == -1 {
        wfifob(src_fd, 35, 0);
        ...
        if (&(*sd).player.inventory.equip)[EQ_HELM as usize].custom_look != 0 {
            wfifob(
                src_fd,
                36,
                (&(*sd).player.inventory.equip)[EQ_HELM as usize].custom_look as u8,
            );
            wfifob(
                src_fd,
                37,
                (&(*sd).player.inventory.equip)[EQ_HELM as usize].custom_look_color as u8,
            );
        } else {
    ...
    let neck_item = if neck_id != 0 {
        Some(item_db::search(neck_id as u32))
    } else {
        None
    };
    ...
    let invis_or_stealth =
        (*sd).player.combat.state == PC_INVIS || ((*sd).optFlags & OPT_FLAG_STEALTH) != 0;
    if invis_or_stealth
        && (*sd).id != (*src_sd).id
        && ((*src_sd).player.identity.gm_level != 0
            || clif_isingroup(src_arc.as_ref(), sd as *mut MapSessionData) != 0
    ...
    if ((*sd).player.identity.gm_level != 0 && (*sd).gfx.toggle != 0) || (*sd).clone != 0 {
        ...
        if (*sd).player.combat.state != PC_INVIS
            && (*sd).player.combat.state != 5
            && gfx_name_len > 0
        {
            len = gfx_name_len;
            wfifob(src_fd, 57, len as u8);
    ...
    // Packet size at [1..2] BE: len + 55
    {
        ...
        if !p.is_null() {
            p.write_unaligned(((len + 55) as u16).to_be());
        }
    }
...
pub unsafe fn clif_parsewalk(pe: &PlayerEntity) -> i32 {
    ...
    let (vx, vy, setting_flags, opt_flags) = {
        ...
        (
            sd.viewx as i32,
            sd.viewy as i32,
            sd.player.appearance.setting_flags,
            sd.optFlags,
        )
    };
    {
    ...
    if dx == sd_x as i32 && dy == sd_y as i32 {
        return 0;
    }
    ...
    for i in 0..MAX_MAGIC_TIMERS {
        let (aether_id, aether_duration) = {
            let sd = pe.read();
            (
                sd.player.spells.dura_aether[i].id,
                sd.player.spells.dura_aether[i].duration,
            )
        };
...
pub unsafe fn clif_noparsewalk(pe: &PlayerEntity, _speed: i8) -> i32 {
    ...
    let Some(md) = map_data(m_val as usize) else {
        return 0;
    };
    ...
    if x0 >= 0 && y0 >= 0 && x0 + (x1 - 1) < md.xs as i32 && y0 + (y1 - 1) < md.ys as i32 {
...
pub unsafe fn clif_parsemap(pe: &PlayerEntity) -> i32 {
    ...
    tracing::debug!(
        "[map] [parsemap] fd={} m={} x0={} y0={} x1={} y1={} check={}",
        fd,
        m,
        x0,
        y0,
...
pub unsafe fn clif_sendmapdata(
    ...
    if x1 * y1 > 323 {
        tracing::warn!(
            "[map] [sendmapdata] fd={} viewport too large x1={} y1={} product={}",
            fd,
            x1,
            y1,
            x1 * y1
    ...
    for y in 0..y1 {
        if y + y0 >= md.ys as i32 {
            break;
        }
        for x in 0..x1 {
            if x + x0 >= md.xs as i32 {
                break;
            }
            ...
            buf2[pos..pos + 2].copy_from_slice(&t.swap_bytes().to_ne_bytes());
            pos += 2;
            buf2[pos..pos + 2].copy_from_slice(&p.swap_bytes().to_ne_bytes());
            pos += 2;
            buf2[pos..pos + 2].copy_from_slice(&o.swap_bytes().to_ne_bytes());
            pos += 2;
    ...
    if checksum == check {
        tracing::debug!(
            "[map] [sendmapdata] fd={} checksum match={} skip send",
            fd,
            checksum
        );
        return 0;
    ...
    tracing::debug!(
        "[map] [sendmapdata] fd={} sending {} bytes computed_check={} client_check={}",
        fd,
        pos,
        checksum,
        check
...
unsafe fn do_warp_check(pe: &crate::game::player::entity::PlayerEntity) {
    ...
    let sd = &mut *sd_ptr;
    let fm = sd.m as i32;
    let Some(fmd) = map_data(fm as usize) else {
        return;
    };
    ...
    let Some(zmd) = map_data(zm as usize) else {
        return;
    };
    ...
    if above_max && sd.player.identity.gm_level == 0 {
        ...
        clif_sendminitext(
            pe,
            c"A magical barrier prevents you from entering.".as_ptr(),
        );
        return;
    }
...
pub unsafe fn clif_object_canmove(m: i32, x: i32, y: i32, side: i32) -> i32 {
    ...
    let Some(flags) = object_flags() else {
        return 0;
    };
...
pub unsafe fn clif_pushback(sd: &mut MapSessionData) -> i32 {
    ...
    match sd.player.combat.side {
        0 => {
            pc_warp(sd_ptr, m, x, y + 2);
        }
        1 => {
            pc_warp(sd_ptr, m, x - 2, y);
        }
        2 => {
            pc_warp(sd_ptr, m, x, y - 2);
        }
...
pub unsafe fn clif_parseviewchange(pe: &PlayerEntity) -> i32 {
    ...
    let x0 = u16::from_be_bytes([rfifob(fd, 8), rfifob(fd, 9)]) as i32;
    let y0 = u16::from_be_bytes([rfifob(fd, 10), rfifob(fd, 11)]) as i32;
...
///
/// Caller must ensure all pointer arguments are valid and non-null.
pub unsafe fn clif_parselookat_scriptsub(_pe: &PlayerEntity) -> i32 {
    0
}
...
pub unsafe fn clif_parselookat(pe: &PlayerEntity) -> i32 {
    let fd = pe.fd;
    let player_id = pe.id;
    let x = u16::from_be_bytes([rfifob(fd, 5), rfifob(fd, 6)]) as i32;
    let y = u16::from_be_bytes([rfifob(fd, 7), rfifob(fd, 8)]) as i32;
    let m = pe.read().m as i32;
    if let Some(grid) = block_grid::get_grid(m as usize) {
