...
pub unsafe fn mob_handle_sub(mob: *mut MobSpawnData) {
    ...
    if in_spawn_window(mob) {
        ...
        if (*mob).last_death + spawn_delay <= sptime {
            ...
            if (*mob).state == MOB_DEAD && (*mob).onetime == 0 {
                ...
                (*mob).attacker = 0;
                let has_users =
                    ffi_map_is_loaded((*mob).bl.m) && crate::game::block::map_user_count((*mob).bl.m as usize) > 0;
                if has_users {
                    mob_respawn(mob);
                } else {
    ...
    let has_users = ffi_map_is_loaded((*mob).bl.m) && crate::game::block::map_user_count((*mob).bl.m as usize) > 0;
                ...
                || ((*mob).newmove > 0 && (*mob).time_ >= (*mob).newmove as i32)
            {
                ...
                if data.r#type == 1 && (*mob).target == 0 {
                    if let Some(grid) = block_grid::get_grid((*mob).bl.m as usize) {
                        ...
                        for id in ids {
                            if let Some(sd_arc) = crate::game::map_server::map_id2sd_pc(id) {
                                rust_mob_find_target_inner(&mut sd_arc.write().bl, mob);
                            }
                        }
                    }
                }
...
pub unsafe fn move_mob(mob: *mut MobSpawnData) -> i32 {
    ...
    if let Some(grid) = block_grid::get_grid(m as usize) {
        ...
        for id in cell_ids {
            if let Some(npc_arc) = crate::game::map_server::map_id2npc_ref(id) {
                rust_mob_move_inner(&mut npc_arc.write().bl, mob);
            }
        }
    }
    ...
    if let Some(grid) = block_grid::get_grid(m as usize) {
        ...
        for cid in cell_ids {
            if let Some(fl_arc) = crate::game::map_server::map_id2fl_ref(cid) {
                rust_mob_addtocurrent_inner(&mut fl_arc.write().bl, def_ptr, id as i32, fl_ptr, sd_ptr);
            }
        }
    }
    ...
    if let Some(grid) = block_grid::get_grid(m as usize) {
        ...
        for id in cell_ids {
            if let Some(ent) = crate::game::map_server::map_id2entity(id) {
                ...
                ent.with_bl_mut(|bl| {
                    rust_mob_move_inner(bl as *mut BlockList, mob);
                });
            }
        }
    }
